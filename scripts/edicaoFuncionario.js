		...
		function executaEdicao(){
			if(verifica()){
				...
				.fail(function(){
					alert("Não foi possivel finalizar a edição, Ocorreu um erro no processo de edição!")
					window.document.getElementById('moldalLoading').style.display = "none"
				})
			}
		}
		...
		function inicia(){
			...
			let c = window.document.getElementById('intituicoesSelecionados').childNodes

			for(i of c){
				if(i.value != null){
					instituicoes.push(i.value)
				}
			}
		...
		function selecionaInstituicao(codigo, texto){
			...
			if(ver){
				...
				window.document.getElementById('intituicoesSelecionados').innerHTML += `<option ondblclick='removeInstituicaoLista(${codigo}, this)'>${texto}</option>`
