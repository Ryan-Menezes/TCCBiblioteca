<?php
	...
	try{
		...
		if(isset($_GET["codU"])){
			$usuario = trim(addslashes($_GET["codU"]));
		}else{
		...
		$comando = $conexao->prepare("SELECT CONCAT(CONCAT(nome, ' '), sobrenome) FROM aluno WHERE id_usuario_aluno = :cod LIMIT 1");
		...
		if(count($dadosUser) > 0){ //Aluno
			$nome = htmlspecialchars($dadosUser[0][0]);
		}else{
			$comando = $conexao->prepare("SELECT CONCAT(CONCAT(nome, ' '), sobrenome) FROM professor WHERE id_usuario_professor = :cod LIMIT 1");
			...
			if(count($dadosUser) > 0){ //Professor
				...
				$comando = $conexao->prepare("SELECT CONCAT(CONCAT(nome, ' '), sobrenome) FROM funcionario WHERE id_usuario_funcionario = :cod LIMIT 1");
		...
		if(count($dadosUser) > 0){
			...
			$comando = $conexao->prepare("SELECT l.cod_livro, l.titulo, l.idioma, l.img_livro, (SELECT AVG(avaliacao_estrelas) FROM avaliacao WHERE livro_tombo_avaliacao = l.cod_livro) FROM livro AS l INNER JOIN lista AS li ON li.livro_tombo_lista = l.cod_livro WHERE li.id_usuario_lista = :cod LIMIT 10");
			...
			if(count($dadosLivro) > 0){
				...
				if(count($dadosLivro) >= 10){
					$html .= "<div id='loadingCont'>
								<div class='carregarMais' title='Carregar Mais Livros' onclick='carregaMaisLivrosLis($usuario, this)'><i class='fas fa-plus'></i></div>
							  </div>";
				}
...
<html lang="pt-br">
<head>
	<title><?php echo Info::$nomeSistema ?> - <?php echo $nome; ?></title>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">

	<script src="./scripts/jquery.min.js"></script>
	<script src="./scripts/MaskaraJquery/jquery.mask.js"></script>
	<script src="./scripts/carregaLivrosLista.js"></script>
