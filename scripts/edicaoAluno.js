		...
		function executaEdicao(){
			if(verifica()){
				window.document.getElementById('cursosSelecionadosT').value = cursos.join()
		...
		function inicia(){
			...
			let c = window.document.getElementById('cursosSelecionados').childNodes
		...
		function selecionaCurso(codigo, texto){
			let ver = true
			...
			if(ver){
				...
				window.document.getElementById('cursosSelecionados').innerHTML += `<option ondblclick='removeCursoLista(${codigo}, this)'>${texto}</option>`

				window.document.getElementsByClassName('MoldalOpcoes')[0].style.display = "none"
			}else{
				alert("Este curso já foi adicionado na lista")
			}
		}
		
		function removeCursoLista(codigo, obj){
			for(let i = 0; i < cursos.length; i++){
				if(cursos[i] == codigo){
					cursos.splice(i, 1)
					obj.remove()
