...
let linhas = []
let colunas = []
let operacaoAluno = null
let operacaoRelatorio = null

function carregaAlunos(btn){
	...
	let info = {
		...
		curso: window.document.getElementById('cursosOP').value.trim(),
		instituicao: window.document.getElementById('instituicaoOP').value.trim(),
		users: linhas.join()
	}

	if(operacaoAluno != null){ 
	    operacaoAluno.abort();
	    operacaoAluno = null;
	}
	...
	.fail(function(){
		if(operacaoAluno == null){
			alert("Ocorreu um erro ao tentar carregar mais alunos")
		}
...
function selecionaInstituicao(obj){
	let select = window.document.getElementById('cursosOP')

	$.ajax({
		method: "POST",
		url: "carregaCursosInstituicao.php",
		data: {cod: obj.value}
	})
	.done(function(res){
...
function excluirAluno(){
	...
	if(input.value.trim().length > 0){
		if(codigoUser != null){
			...
			.fail(function(){
				alert("Aluno não deletado, Ocorreu um erro na operação de exclusão")
...
function abreMoldalRelatorio(){
	if(linhas.length > 0){
		...
		alert("Selecione pelo menos um aluno para emitir o relatório!")
	}
}
