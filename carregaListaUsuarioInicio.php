<?php
	...
	try{
		...
		if(isset($_POST["cod"]) && isset($_POST["min"])){
			...
			$comando = $conexao->prepare("SELECT l.cod_livro, l.titulo, l.idioma, l.img_livro, (SELECT AVG(avaliacao_estrelas) FROM avaliacao WHERE livro_tombo_avaliacao = l.cod_livro) FROM livro AS l INNER JOIN lista AS li ON li.livro_tombo_lista = l.cod_livro WHERE li.id_usuario_lista = :cod LIMIT $minimo, 10");
			...
			if(count($dadosLivro) > 0){
				...
				if(count($dadosLivro) >= 10){
					$html .= "<div id='loadingCont'>
								<div class='carregarMais' title='Carregar Mais Livros' onclick='carregaMaisLivrosLis($codigo, this)'><i class='fas fa-plus'></i></div>
							  </div>";
				}
