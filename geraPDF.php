<?php
	...
	if(isset($_POST["tipo"]) && isset($_POST["columns"]) && isset($_POST["rows"]) && isset($_POST["title"]) && isset($_POST["tp"]) && isset($_POST["op"])){
		...
		$tamanho = trim(addslashes($_POST["tp"]));
		$orientacao = trim(addslashes($_POST["op"]));
		...
		if($tipo == 1){ //1 - Alunos | 2 - Professores | 3 - Funcionários | 4 - Livros
			...
			$html = "<!DOCTYPE html>
					 ...
					 <table>
						<thead>
							<tr class='thHeader' style='text-align: left'>
								<th colspan='3'>
									<img src='". Info::$logoEtec ."'>
									<img src='". Info::$logoSistema ."'>
									<img src='". Info::$logoCentro ."'>
								</th>
								<th colspan='".((count($columns) > 3) ? count($columns) - 3 : count($columns))."' style='text-align: left'>$titulo</th>
							</tr>
							<tr>";
			...
			foreach($dados as $valor){
				...
				for($i = 0; $i < (count($valor)/2); $i++){
					if($columns[$i] == "0"){
						$html .= "<td><img src='data:image/jpg;base64,".base64_encode($valor[$i])."'></td>";
					}else if($columns[$i] == "3"){ //CPF
					 ...
					 <table style='text-align: center'>
						<thead>
							<tr class='thHeader'>
								...
								<th colspan='".((count($columns) > 2) ? count($columns) - 2 : count($columns))."' style='text-align: left'>$titulo</th>
							</tr>
							<tr>";
					 ...
					 <table style='text-align: center'>
						<thead>
							<tr class='thHeader'>
								...
								<th colspan='".(count($columns) - 2)."' style='text-align: left'>$titulo</th>
							</tr>
							<tr>";
			...
			foreach($dados as $valor){
				...
				for($i = 0; $i < count($columns); $i++){
					if($columns[$i] == "0"){
						$img = base64_encode($valor[$i]);
						$html .= "<td><img src='data:image/jpg;base64,$img'></td>";
					}else if($columns[$i] == "5"){ //Autores
				...
				foreach($dados as $valor){
					...
					for($i = 0; $i < count($columns); $i++){
						if($columns[$i] == "1"){
							$img = base64_encode($valor[$i]);
							$html .= "<td><img src='data:image/jpg;base64,$img'></td>";
						}else if($columns[$i] == "3" || $columns[$i] == "4" || $columns[$i] == "5"){ //Usuário
